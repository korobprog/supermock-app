...
export default function PricingClient() {
  ...
  // Helper function to get features from individual keys
  const getFeatures = (planKey: string): string[] => {
    const features: string[] = [];
    let index = 1;
    
    // Define the expected number of features for each plan
    const maxFeatures: Record<string, number> = {
      'pricing.free': 3,
      'pricing.basic': 4,
      'pricing.pro': 6
    };
    
    const maxFeatureCount = maxFeatures[planKey] || 10; // fallback to 10 if not found
    
    while (index <= maxFeatureCount) {
      try {
        const featureKey = `${planKey}.feature${index}`;
        const feature = t(featureKey);
        // Check if the translation exists and is not the key itself
        if (feature && feature !== featureKey && !feature.includes('MISSING_MESSAGE')) {
          features.push(feature);
        }
        index++;
      } catch (error) {
        // If we get an error, it means the key doesn't exist, so we stop
        break;
      }
    }
  ...
  const plans = [
    {
      ...
      included: getFeatures('pricing.free'),
      excluded: [
        t('features.ai.title'),
    ...
    },
    {
      ...
      included: getFeatures('pricing.basic'),
      excluded: [
        t('learningProcess.processSteps.step7.title'),
        t('learningProcess.processSteps.step7.description'),
            ...
            <CardContent className="p-12">
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <ClientButton variant="outline" size="xl" href="/learning-process">
